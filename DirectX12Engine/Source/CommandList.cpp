...
std::map<std::wstring, ID3D12Resource*> dx12lib::CommandList::ms_TextureCache;
std::mutex                              dx12lib::CommandList::ms_TextureCacheMutex;

dx12lib::CommandList::CommandList( Device& device, D3D12_COMMAND_LIST_TYPE type )
: m_Device( device )
...
void dx12lib::CommandList::TransitionBarrier( COMPTR<ID3D12Resource> resource, D3D12_RESOURCE_STATES stateAfter,
...
void dx12lib::CommandList::TransitionBarrier( const std::shared_ptr<Resource>& resource, D3D12_RESOURCE_STATES stateAfter,
                                     UINT subresource, bool flushBarriers )
{
    if ( resource )
    {
...
void dx12lib::CommandList::UAVBarrier( COMPTR<ID3D12Resource> resource, bool flushBarriers )
{
...
void dx12lib::CommandList::UAVBarrier( const std::shared_ptr<Resource>& resource, bool flushBarriers )
{
    auto d3d12Resource = resource ? resource->GetD3D12Resource() : nullptr;
    UAVBarrier( d3d12Resource, flushBarriers );
}

void dx12lib::CommandList::AliasingBarrier( COMPTR<ID3D12Resource> beforeResource,
                                   COMPTR<ID3D12Resource> afterResource, bool flushBarriers )
{
...
void dx12lib::CommandList::AliasingBarrier( const std::shared_ptr<Resource>& beforeResource,
...
void dx12lib::CommandList::FlushResourceBarriers()
{
    m_ResourceStateTracker->FlushResourceBarriers( shared_from_this() );
}

void dx12lib::CommandList::CopyResource( COMPTR<ID3D12Resource> dstRes,
...
void dx12lib::CommandList::CopyResource( const std::shared_ptr<Resource>& dstRes, const std::shared_ptr<Resource>& srcRes )
{
...
void dx12lib::CommandList::ResolveSubresource( const std::shared_ptr<Resource>& dstRes, const std::shared_ptr<Resource>& srcRes,
...
COMPTR<ID3D12Resource> dx12lib::CommandList::CopyBuffer( size_t bufferSize, const void* bufferData, D3D12_RESOURCE_FLAGS flags )
{
...
std::shared_ptr<VertexBuffer> dx12lib::CommandList::CopyVertexBuffer( size_t numVertices, size_t vertexStride,
...
std::shared_ptr<IndexBuffer> dx12lib::CommandList::CopyIndexBuffer( size_t numIndices, DXGI_FORMAT indexFormat,
...
std::shared_ptr<ConstantBuffer> dx12lib::CommandList::CopyConstantBuffer( size_t bufferSize, const void* bufferData )
{
...
std::shared_ptr<ByteAddressBuffer> dx12lib::CommandList::CopyByteAddressBuffer( size_t bufferSize, const void* bufferData )
{
...
std::shared_ptr<StructuredBuffer> dx12lib::CommandList::CopyStructuredBuffer( size_t numElements, size_t elementSize,
...
void dx12lib::CommandList::SetPrimitiveTopology( D3D_PRIMITIVE_TOPOLOGY primitiveTopology )
{
    m_d3d12CommandList->IASetPrimitiveTopology( primitiveTopology );
}

std::shared_ptr<Texture> dx12lib::CommandList::LoadTextureFromFile( const std::wstring& fileName, bool sRGB )
{
...
void dx12lib::CommandList::GenerateMips( const std::shared_ptr<Texture>& texture )
{
    if ( !texture )
        return;
...
void dx12lib::CommandList::GenerateMips_UAV( const std::shared_ptr<Texture>& texture, bool isSRGB )
{
...
void dx12lib::CommandList::PanoToCubemap( const std::shared_ptr<Texture>& cubemapTexture,
...
std::shared_ptr<Scene> dx12lib::CommandList::LoadSceneFromFile( const std::wstring&                 fileName,
...
std::shared_ptr<Scene> dx12lib::CommandList::LoadSceneFromString( const std::string& sceneString, const std::string& format )
{
    auto scene = std::make_shared<Scene>();
...
std::shared_ptr<Scene> dx12lib::CommandList::CreateScene( const VertexCollection& vertices, const IndexCollection& indices )
{
    if ( vertices.empty() )
    {
        return nullptr;
...
std::shared_ptr<Scene> dx12lib::CommandList::CreateCube( float size, bool reverseWinding )
{
...
std::shared_ptr<Scene> dx12lib::CommandList::CreateSphere( float radius, uint32_t tessellation, bool reversWinding )
{
...
void dx12lib::CommandList::CreateCylinderCap( VertexCollection& vertices, IndexCollection& indices, size_t tessellation,
...
std::shared_ptr<Scene> dx12lib::CommandList::CreateCylinder( float radius, float height, uint32_t tessellation,
                                                    bool reverseWinding )
{
    if ( tessellation < 3 )
        throw std::out_of_range( "tessellation parameter out of range" );
...
std::shared_ptr<Scene> dx12lib::CommandList::CreateCone( float radius, float height, uint32_t tessellation, bool reverseWinding )
{
    if ( tessellation < 3 )
        throw std::out_of_range( "tessellation parameter out of range" );
...
std::shared_ptr<Scene> dx12lib::CommandList::CreateTorus( float radius, float thickness, uint32_t tessellation,
                                                 bool reverseWinding )
{
...
std::shared_ptr<Scene> dx12lib::CommandList::CreatePlane( float width, float height, bool reverseWinding )
{
...
void dx12lib::CommandList::ClearTexture( const std::shared_ptr<Texture>& texture, const float clearColor[4] )
{
    assert( texture );
...
void dx12lib::CommandList::ClearDepthStencilTexture( const std::shared_ptr<Texture>& texture, D3D12_CLEAR_FLAGS clearFlags,
...
void dx12lib::CommandList::CopyTextureSubresource( const std::shared_ptr<Texture>& texture, uint32_t firstSubresource,
...
void dx12lib::CommandList::SetGraphicsDynamicConstantBuffer( uint32_t rootParameterIndex, size_t sizeInBytes,
                                                    const void* bufferData )
{
...
void dx12lib::CommandList::SetGraphics32BitConstants( uint32_t rootParameterIndex, uint32_t numConstants, const void* constants )
{
    m_d3d12CommandList->SetGraphicsRoot32BitConstants( rootParameterIndex, numConstants, constants, 0 );
}

void dx12lib::CommandList::SetCompute32BitConstants( uint32_t rootParameterIndex, uint32_t numConstants, const void* constants )
{
    m_d3d12CommandList->SetComputeRoot32BitConstants( rootParameterIndex, numConstants, constants, 0 );
}

void dx12lib::CommandList::SetVertexBuffers( uint32_t                                          startSlot,
...
void dx12lib::CommandList::SetVertexBuffer( uint32_t slot, const std::shared_ptr<VertexBuffer>& vertexBuffer )
{
    SetVertexBuffers( slot, { vertexBuffer } );
}

void dx12lib::CommandList::SetDynamicVertexBuffer( uint32_t slot, size_t numVertices, size_t vertexSize,
                                          const void* vertexBufferData )
{
...
void dx12lib::CommandList::SetIndexBuffer( const std::shared_ptr<IndexBuffer>& indexBuffer )
{
    if ( indexBuffer )
    {
...
void dx12lib::CommandList::SetDynamicIndexBuffer( size_t numIndices, DXGI_FORMAT indexFormat, const void* indexBufferData )
{
...
void dx12lib::CommandList::SetGraphicsDynamicStructuredBuffer( uint32_t slot, size_t numElements, size_t elementSize,
                                                      const void* bufferData )
{
...
void dx12lib::CommandList::SetViewport( const D3D12_VIEWPORT& viewport )
{
    SetViewports( { viewport } );
}

void dx12lib::CommandList::SetViewports( const std::vector<D3D12_VIEWPORT>& viewports )
{
    assert( viewports.size() < D3D12_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE );
    m_d3d12CommandList->RSSetViewports( static_cast<UINT>( viewports.size() ), viewports.data() );
}

void dx12lib::CommandList::SetScissorRect( const D3D12_RECT& scissorRect )
{
    SetScissorRects( { scissorRect } );
}

void dx12lib::CommandList::SetScissorRects( const std::vector<D3D12_RECT>& scissorRects )
{
...
void dx12lib::CommandList::SetPipelineState( const std::shared_ptr<PipelineStateObject>& pipelineState )
{
...
void dx12lib::CommandList::SetGraphicsRootSignature( const std::shared_ptr<RootSignature>& rootSignature )
{
...
void dx12lib::CommandList::SetComputeRootSignature( const std::shared_ptr<RootSignature>& rootSignature )
{
    assert( rootSignature );
...
void dx12lib::CommandList::SetConstantBufferView( uint32_t rootParameterIndex, const std::shared_ptr<ConstantBuffer>& buffer,
...
void dx12lib::CommandList::SetShaderResourceView( uint32_t rootParameterIndex, const std::shared_ptr<Buffer>& buffer,
                                         D3D12_RESOURCE_STATES stateAfter, size_t bufferOffset )
{
    if ( buffer )
    {
...
void dx12lib::CommandList::SetUnorderedAccessView( uint32_t rootParameterIndex, const std::shared_ptr<Buffer>& buffer,
                                          D3D12_RESOURCE_STATES stateAfter, size_t bufferOffset )
{
    if ( buffer )
    {
...
void dx12lib::CommandList::SetShaderResourceView( uint32_t rootParameterIndex, uint32_t descriptorOffset,
...
void dx12lib::CommandList::SetShaderResourceView( int32_t rootParameterIndex, uint32_t descriptorOffset,
...
void dx12lib::CommandList::SetUnorderedAccessView( uint32_t rootParameterIndex, uint32_t descriptorOffset,
...
void dx12lib::CommandList::SetConstantBufferView( uint32_t rootParameterIndex, uint32_t descriptorOffset,
...
void dx12lib::CommandList::SetRenderTarget( const RenderTarget& renderTarget )
{
...
void dx12lib::CommandList::Draw( uint32_t vertexCount, uint32_t instanceCount, uint32_t startVertex, uint32_t startInstance )
{
    FlushResourceBarriers();
...
void dx12lib::CommandList::DrawIndexed( uint32_t indexCount, uint32_t instanceCount, uint32_t startIndex, int32_t baseVertex,
...
void dx12lib::CommandList::Dispatch( uint32_t numGroupsX, uint32_t numGroupsY, uint32_t numGroupsZ )
{
    FlushResourceBarriers();
...
bool dx12lib::CommandList::Close( const std::shared_ptr<dx12lib::CommandList>& pendingCommandList )
{
...
void dx12lib::CommandList::Close()
{
    FlushResourceBarriers();
    m_d3d12CommandList->Close();
}

void dx12lib::CommandList::Reset()
{
...
void dx12lib::CommandList::TrackResource( COMPTR<ID3D12Object> object )
{
    m_TrackedObjects.push_back( object );
}

void dx12lib::CommandList::TrackResource( const std::shared_ptr<Resource>& res )
{
    assert( res );
...
void dx12lib::CommandList::ReleaseTrackedObjects()
{
    m_TrackedObjects.clear();
}

void dx12lib::CommandList::SetDescriptorHeap( D3D12_DESCRIPTOR_HEAP_TYPE heapType, ID3D12DescriptorHeap* heap )
{
...
void dx12lib::CommandList::BindDescriptorHeaps()
{
